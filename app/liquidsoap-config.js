...
const { scheduleRanges } = require("./playlist-schedule");
...
function normalisePlaylistSources(config) {
    ...
    return rawSources.map((source, index) => {
        ...
        return {
            id: source.id, playlistPath: source.playlistPath, weight,
            ranges: scheduleRanges(source.schedule, `${label}.schedule`),
        };
    });
}
...
function generateScript(config) {
    ...
    const lines = [
        ...
        "# Edit autodj.config.json or playlist.config.json and restart AutoDJ",
    ...
    for (const program of programs) {
        ...
        lines.push(
            ...
            ...(sources.some((source) => source.ranges.length > 0) ? [
                `  schedules=[${sources.map((source) => `[${source.ranges.map((range) => `{start=${range.start}, stop=${range.end}}`).join(", ")}]`).join(", ")}],`,
            ] : []),
